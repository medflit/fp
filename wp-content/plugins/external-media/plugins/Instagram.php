 ...
 */
class InstagramPhotoPicker extends WPUploadMedia_Base {
  ...
  public function redirectContents() {
    ...
    // Instagram photos.
    $photos = $this->instagramUserPhotos( $access_token );
    // Currenly logged in user profile.
    $profile = $this->instagramUserProfile( $access_token );
    $counts = array(
    ...
    return array(
      ...
      'head'    => '<link rel="stylesheet" type="text/css" href="' . plugins_url( '/css/instagram-popup.css', WP_ExternalMedia_PluginName ) . '">',
      'content' => '',
      'footer'  => '<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
        <script src="' . plugins_url( '/js/instagram-popup.js', WP_ExternalMedia_PluginName ) . '"></script>',
    );
  }
   ...
   */
  protected function instagramUserPhotosAll($url, $limit) {
    ...
    if ( count( $this->photos ) <= $limit && !empty( $media->pagination->next_url ) ) {
      ...
      return $this->instagramUserPhotosAll(
   ...
   * Get all user photos.
   */
  protected function instagramUserPhotos( $access_token ) {
    ...
    if ( $access_token ) {
      $photos = wp_cache_get( 'photos', get_class($this), 3600 );
      if ( false === $photos ) {
        // Get the most recent media published by a user.
        $photos = $this->instagramUserPhotosAll(
          $this->api_endpoint . '/users/self/media/recent/?access_token=' . $access_token
          . '&count=' . $limit,
          $limit
        );
        wp_cache_set( 'photos', $photos, get_class($this) );
      }
    }
   ...
   * Profile information.
   */
  protected function instagramUserProfile( $access_token ) {
    if ( $access_token ) {
