...
jQuery(function ($) {
  ...
  var _multiselect = ( _cardinality > 1 ) ? true : false;
    ...
    if ( !$( this ).hasClass( 'disabled' ) ) {
      if ( selected.length ) {
        ...
        for ( var i = 0; i < selected.length; i++ ) {
          var _item = $( '#photos a[data-index="' + selected[i] + '"]' ).data( 'standard-img' );
          _item += '::' + $( '#photos a[data-index="' + selected[i] + '"]' ).data( 'link' );
          _files.push( _item );
        }
